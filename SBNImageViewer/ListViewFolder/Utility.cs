...
namespace Sbn.AdvancedControls.Imaging.ImageViewer.ListViewFolder
{
    ...
    public static class Utility
    {
        ...
        internal class ShellImageFileInfo
        {
            ...
            public static  bool UpdateFileInfo(string path , int PropertyTag , object newValue)
            {
              //  Encoding.ASCII.GetString(value, 0, len);
                  ...
                  //  TypeName = typeName;
                    using (var stream = new FileStream(path, FileMode.Open, FileAccess.ReadWrite))
                    {
                        using (Image img = Image.FromStream(stream, false, false))
                        {
                            ...
                            try
                            {
                                //var pi = img.PropertyItems[0];
                                //pi.Id = PropertyTag;
                                //pi.Value = Encoding.ASCII.GetBytes(newValue.ToString());
                                //img.PropertyItems.SetValue(newValue, img.PropertyItems.Length);
                                //img.Save(path);
                            }
                            catch 
                            {
        ...
        /// <param name="backColor">Background color of returned thumbnail.</param>
        /// <returns>The image from the given file or null if an error occurs.</returns>
        internal static Image ThumbnailFromFile2(string filename, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, Color backColor)
        {
            if (size.Width <= 0 || size.Height <= 0)
                throw new ArgumentException();
        ...
        /// <param name="backColor">Background color of returned thumbnail.</param>
        /// <returns>The image from the given file or null if an error occurs.</returns>
        internal static Image ThumbnailFromFile(ImageListItem imglsvItm, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, Color backColor)
        {
            if (size.Width <= 0 || size.Height <= 0)
                throw new ArgumentException();


            //if (imglsvItm.Stream == null || imglsvItm.Stream.Length < 10)
            //     imglsvItm.OnNeedDrawImage(new ImageLisViewItemEventArg(imglsvItm));
            ...
            // Revert to source image if an embedded thumbnail of required size
            // was not found.
            //if (imglsvItm.Stream != null)
            //{
            //    try
            //    {
            //        using (var ms = new MemoryStream(imglsvItm.Stream))
            //        {
            ...
            if (useEmbeddedThumbnails != UseEmbeddedThumbnails.Never)
            {
                try
                {
                    using (FileStream stream = new FileStream(imglsvItm.FileName, FileMode.Open, FileAccess.Read))
                    {
                        using (Image img = Image.FromStream(stream, false, false))
                        {
                            foreach (int index in img.PropertyIdList)
            ...
            {
                Size scaled = GetSizedImageBounds(source, size);
               // Size scaled = size;// GetSizedImageBounds(source, size);
                thumb = new Bitmap(source, scaled.Width, scaled.Height);
