 ...
 //   [DesignerSerializer(typeof(ImageListViewSerializer), typeof(CodeDomSerializer))]
  //  [Docking(DockingBehavior.Ask)]
    public partial class ImageListView : Control
    {
        ...
        internal ImageListItem mSelectedItem;
        ...
        //private ImageListViewItem _viewItem = null;
        //public ImageListViewItem ViewItem {get { return _viewItem; }
        //    set
        //    {
        //        if (value != _viewItem)
        //        {
        ...
        public ImageListItem SelectedItem { get { return mSelectedItem; } }
        /// <summary>
        ...
        /// </summary>
        public Point ViewOffset { get { return mViewOffset; } set
        {

           // mViewOffset = value; Commented by rm
            if (value.X >= hScrollBar.Maximum)
            {
                hScrollBar.Value = hScrollBar.Maximum; //added by rm
            }
            else if (value.X < hScrollBar.Minimum)
            {
                hScrollBar.Value = 0; //added by rm
            }
            else
            {
                hScrollBar.Value = value.X; //added by rm
            }

            if (value.Y >= vScrollBar.Maximum)
            {
                vScrollBar.Value = vScrollBar.Maximum; //added by rm
            }
            else if (value.Y< vScrollBar.Minimum)
            {
                vScrollBar.Value = 0; //added by rm
            }
            else
            {
                vScrollBar.Value = value.Y; //added by rm
            }


            mViewOffset = new Point(hScrollBar.Value, vScrollBar.Value); //added by rm
        } }
        /// <summary>
        ...
        internal ScrollOrientation ScrollOrientation
        {
            get
            {
                return (mView == View.Gallery ? ScrollOrientation.HorizontalScroll : ScrollOrientation.VerticalScroll);
            }
        }
        /// <summary>
        ...
        public ImageListView()
        {
            ...
            ResourceManager manager = new ResourceManager("Sbn.AdvancedControls.Imaging.ImageViewer.ListViewFolder.ImageListViewResources",
        ...
        public ItemVisibility IsItemVisible(ImageListItem item)
        {
        ...
        public event EventHandler<ImageLisViewItemEventArg> NeedDrawImage;
        /// <summary>
        ...
        protected override void OnResize(EventArgs e)
        {
          ...
          //  ThumbnailSize = new Size(Size.Width - 20, ThumbnailSize.Height);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            try
            {
                if (e.Button == MouseButtons.Left && (_OldPosition.X > 0 || _OldPosition.Y > 0))
                {
                    int deltaY = _OldPosition.Y - e.Y;
                    if (deltaY == 0)
                        return;
                    mRenderer.SuspendPaint();
                    if (ScrollOrientation == ScrollOrientation.VerticalScroll)
                    {
                        ...
                        int newYOffset = mViewOffset.Y + (deltaY); //*vScrollBar.SmallChange;
                        ...
                        int newXOffset = mViewOffset.X + (_OldPosition.X - e.X);
                        if (newXOffset > hScrollBar.Maximum - hScrollBar.LargeChange + 1)
                            newXOffset = hScrollBar.Maximum - hScrollBar.LargeChange + 1;
                        if (newXOffset < 0)
                            newXOffset = 0;
                    ...
                    {
                        int newXOffset = mViewOffset.X + (deltaY);
                        if (newXOffset > hScrollBar.Maximum - hScrollBar.LargeChange + 1)
                            newXOffset = hScrollBar.Maximum - hScrollBar.LargeChange + 1;
                        if (newXOffset < 0)
                            newXOffset = 0;
        ...
        protected override void OnMouseWheel(MouseEventArgs e)
        {
            ...
            if (ScrollOrientation == ScrollOrientation.VerticalScroll)
            {
                int newYOffset = mViewOffset.Y - (e.Delta / 120) * vScrollBar.SmallChange/10;
                if (newYOffset > vScrollBar.Maximum - vScrollBar.LargeChange + 1)
                    newYOffset = vScrollBar.Maximum - vScrollBar.LargeChange + 1;
                if (newYOffset < 0)
                    newYOffset = 0;
        ...
        protected virtual void OnDropFiles(DropFileEventArgs e)
        {
            ...
            {
                ImageListItem item = new ImageListItem(filename);
                item.mSelected = true;
        ...
        public void OnViewItemChanged(EventArgs e , ImageListItem itm)
        {
            EventHandler handler = ViewItemChanged;
            if (handler != null) if (itm != null) handler(itm, e);
        }
        /// <summary>
        ...
        /// </summary>
        internal void UpdateItemDetailsInternal(ImageListItem item, VirtualItemDetailsEventArgs info)
        {
        ...
        [Category("Behavior"), Browsable(true), Description("Occurs when the view item.")]
        public event EventHandler ViewItemChanged;
        ...
        //public void FocusedItem(ImageListViewItem itm)
        //{
        //    if (itm != null) EnsureVisible(itm.Index);
        //    return;
