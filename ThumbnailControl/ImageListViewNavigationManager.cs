...
namespace Sbn.AdvancedControls.Imaging.ThumbnailControl
{
    public partial class ImageListView
    {
        ...
        internal class ImageListViewNavigationManager : IDisposable
        {
            ...
            public void MoveItems(MovedItemsEventArgs e)
            {

                mImageListView.OnMovedItems(e);

                if (e.Cancel)
                    return;


                List<ImageListViewItem> draggedItems = e.Items;// new List<ImageListViewItem>();
                int i = -1;
                if (DropTarget != null) i = e.Index;// DropTarget.Index;
                foreach (ImageListViewItem item in mImageListView.SelectedItems)
                {
                    if (item.Index <= i) i--;
            ...
            public void DragDrop(DragEventArgs e)
            {
                ...
                if (selfDragging)
                {
                    ...
                    MovedItemsEventArgs ee = new MovedItemsEventArgs(DropTarget.Index, draggedItems, IsDropToRight, DropToRight);
                    MoveItems(ee);
                    //if (i < 0) i = 0;
                    //if (i > mImageListView.Items.Count - 1) i = mImageListView.Items.Count - 1;
                    //if (DropToRight) i++;
                    ...
                    //    i++;
                    //}
                    //mImageListView.OnMovedItems(new MovedItemsEventArgs(DropTarget.Index, draggedItems));
                    //mImageListView.OnSelectionChangedInternal();
